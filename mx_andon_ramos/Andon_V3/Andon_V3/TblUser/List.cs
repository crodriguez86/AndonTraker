...
namespace Andon_V3.TblUser
{
    public partial class List : Form
    {
        ...
        private void loadData()
        {
            ...
            {
                MreaMessage.ShowWarning("Can't load users from database. " + objBLL._error);
            }
            ...
            if (e.RowIndex >= 0)
            {
...

                {
                    ...
                    else if (grdData[e.ColumnIndex, e.RowIndex].Value.ToString() == "Delete")
                    {
                        ...
                        {
                            UserBLL objBLL = new UserBLL();
                            //No dejar eliminar si solo hay un usuario registrado.
                            ...
                            if (users != null)
                            {
                                ...
                                {
                                    MreaMessage.ShowWarning("Can't delete last user. You must create a new user and delete this one.");
                                }
                            }
                            else
                            {
        ...
        private void grdLines_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            {
                grdData[4, i].Value = "Update";
                grdData[5, i].Value = "Delete";
            }
        }
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (list != null)
            {
                list = list.Where(p => p.AuthName.Contains(textBox1.Text)).ToList();
                grdData.DataSource = list;
            }
            else
            {
