...
namespace Andon_V3.PLCs
{
    public partial class List : Form
    {
        ...
        {
            AndonPlcBLL lineBLL = new AndonPlcBLL();
            grdData.AutoGenerateColumns = false;
            List<AndonPlc> list = lineBLL.getAndonPlc(new AndonPlc());
            if (list != null)
            {
            ...
            {
                ShowWarning("No se pudo cargar PLC's de base datos");
            }
        ...
        private void grdLines_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
...

                {
                    ...
                    {
                        AndonPlcBLL lineBLL = new AndonPlcBLL();
                        if (lineBLL.deleteAndonPlc(Convert.ToInt32(val)))
        ...
        private void grdLines_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            {
                grdData[3, i].Value = "Update";
                grdData[4, i].Value = "Delete";
            }
        }
