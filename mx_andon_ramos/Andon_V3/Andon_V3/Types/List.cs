...
namespace Andon_V3.Types
{
    public partial class List : Form
    {
        ...
        {
            AndonTypeBLL lineBLL = new AndonTypeBLL();
            grdData.AutoGenerateColumns = false;
            List<AndonType> list = lineBLL.getAndonType(new AndonType());
            if (list != null)
            {
            ...
            {
                ShowWarning("No se pudo cargar Tipos de base datos");
            }
            ...
            if (e.RowIndex >= 0)
            {
...

                {
                    ...
                    else if (grdData[e.ColumnIndex, e.RowIndex].Value.ToString() == "Delete")
                    {
                        ...
                        {
                            AndonTypeBLL lineBLL = new AndonTypeBLL();
                            if (lineBLL.deleteAndonType(Convert.ToInt32(val)))
                                ShowOK("Deleted successfully!");
                            else
                                ShowWarning("Something went wrong. " + lineBLL._error);
                        }
                        loadData();
                    }
        ...
        private void grdLines_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            {
                grdData[13, i].Value = "Update";
                grdData[14, i].Value = "Delete";
            }
        }
