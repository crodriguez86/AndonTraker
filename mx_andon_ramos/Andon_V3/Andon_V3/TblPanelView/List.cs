...
namespace Andon_V3.TblPanelView
{
    public partial class List : Form
    {
        ...
        private void loadData()
        {
            ...
            List<AndonPanelView> list = objBLL.GetAllByIdGroup(_id);
            if (list != null)
            {
            ...
            {
                ShowWarning("Can't load panel views from database. " + objBLL._error);
            }
        ...
        private void grdLines_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
...

                {
                    ...
                    {
                        int id = Convert.ToInt32(val);
                        TblPanelButton.List listButtons = new TblPanelButton.List();
                        listButtons._id = id;
                        var screen = Screen.FromPoint(Cursor.Position);
                        listButtons.StartPosition = FormStartPosition.Manual;
                        listButtons.Left = screen.Bounds.Left + screen.Bounds.Width / 2 - listButtons.Width / 2;
                        listButtons.Top = screen.Bounds.Top + screen.Bounds.Height / 2 - listButtons.Height / 2;
                        listButtons.ShowDialog();
                    }
                }
            }
        }
        private void grdLines_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            ...
            {
                grdData[6, i].Value = "Update";
                grdData[7, i].Value = "Delete";
                grdData[8, i].Value = "View";
            }
        }
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            ...
            if (list != null)
            {
                list = list.Where(p => p.PanelName.Contains(textBox1.Text)).ToList();
                grdData.DataSource = list;
            }
            else
            {
